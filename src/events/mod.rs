...
/// A handler function type for events.
type EventHandler<T> = Box<dyn Fn(T) + Send + Sync + 'static>;

/// Thread-safe event emitter for a specific event type.
#[derive(Clone)]
...
/// Events collector for PeerConnection.
///
/// Aggregates all W3C-compatible event emitters.
#[derive(Clone)]
